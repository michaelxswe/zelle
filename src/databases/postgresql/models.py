...
class TransactionModel(Base):
    ...
    receiver_id: Mapped[int | None] = mapped_column(ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
...
class UserModel(Base):
    ...
    balance: Mapped[Decimal] = mapped_column(
        NUMERIC(precision=10, scale=2), server_default="0.00", init=False
    )
    created_date: Mapped[datetime] = mapped_column(
        TIMESTAMP(timezone=True), server_default=text("date_trunc('s', now())"), init=False
    )
